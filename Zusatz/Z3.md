...
Projektseite: https://z3r0degree.github.io/M324_gruppe4/
...
- Anwendung: Node/Express Backend im `Code/` Verzeichnis wurde mit `prom-client` instrumentiert und gibt strukturierte Logs (JSON) per `winston` aus.
...
Hinweis: Diese Pakete sind bereits im Projekt installiert (siehe `package.json`).

- Beispiel: `metrics` Endpoint in `index.js` hinzufügen:
...
Hinweis: Diese Konfiguration ist bereits in `docker-compose.monitoring.yml` vorhanden.
...
4. Trigger CI (Push / PR). Zeige im GitHub Actions Lauf, dass Tests erfolgreich durchlaufen und Coverage hochgeladen wird. Link zur Pages‑Seite (Coverage) = https://z3r0degree.github.io/M324_gruppe4/
